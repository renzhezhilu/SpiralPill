SP_getCalendar({
	whatDate:null,				//默认日期 默认今日
	className:".SP_calendar",	//容器
	lunarDayShow:null,			//农历显示 默认隐藏
	holiSayShow:null  			//节日显示 默认显示
})

//获取日历
function SP_getCalendar(set) {
	//设置参数
	var whatDate, className , lunarDayShow;
	whatDate = set.whatDate ? set.whatDate : new Date();
	className = set.className;
	lunarDayShow = set.lunarDayShow===true ? true:false;
	holiSayShow= set.holiSayShow===false ? false:true;
	...
	!function () {
		...
		for (var i = start; i < (daynum + end); i++) {
			var all = new Date(whatDate);
			all.setDate(i)
			var li_date = all.toLocaleDateString()
			var icon = '';
			//农历展示
			var lunarDay = lunarDayShow ? dateToLunar(all).lunarDay:'';
			var typetxt = ''; 
			//判断是否节日
			if (ifHoliday(all)&&holiSayShow) {
				icon = "<i class='fa fa-bookmark'></i>";
				lunarDay = ifHoliday(all);
			}
			//非本月
			if (i <= 0 || i > daynum) {
				m_class = 'othermonth'
			...
			//指定日/今日
			else if (new Date().toLocaleDateString() == all.toLocaleDateString()) {
				m_class = 'nowmonth on';
				lunarDay = '今天'
			}
			//本月
		...
		! function () {
		    ...
		    <div class='left' >\
					<h1>"+$(className).attr("data-tit")+"</h1>\
				</div>	\
				<h2 class='dateshow right'>" + now_Y + "年" + (now_M + 1) + "月</h2>\
				<div class='datebut right' >\
					<span class='prvebut'>\
			...
			$(className).find('.nextbut').click(function () {
				SP_getCalendar({
					whatDate:new Date(now_Y, now_M + 1),				
					className:className	
				})
			});
